...
export default function CharityFeedPage() {
  ...
  // Fetch current charity identity
  useEffect(() => {
    charityApi.get("/api/charity/profile").then((res) => {
      const d = res.data?.data;
      if (d) setMe({ id: d.userId ?? d.id, name: d.name, role: "CHARITY", charityAccount: { name: d.name, logoUrl: d.logoUrl } });
    }).catch(() => {});
  }, []);
  ...
  const loadPosts = useCallback(async (p: number, replace = false) => {
    ...
    try {
      const res = await charityApi.get(`/api/posts?page=${p}&limit=${limit}`);
  ...
  const charityName = me?.charityAccount?.name ?? me?.name ?? "";
  const charityLogo = me?.charityAccount?.logoUrl;
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      {me && (
        ...
        >
          ...
          <span className="text-sm text-gray-400">Share a project update, announcement or achievement…</span>
        </button>
      )}
          ...
          {hasMore && (
            <div className="text-center pt-2">
              <button
                ...
                className="px-6 py-2.5 text-sm font-medium text-emerald-600 border border-emerald-200 rounded-xl hover:bg-emerald-50 transition-colors disabled:opacity-50"
              >
